...
- `AIRadio.sln` と各プロジェクト:
  ...
  - `AIRadio.App`（Avalonia executable, → Core/Infrastructure。最小スタブ。トレイ UI は W9）
  - `AIRadio.Core.Tests` / `AIRadio.Infrastructure.Tests`（xUnit）
- `AIRadio.Core`:
  - interface: `ILLMBackend` / `ITTSBackend` / `IAudioPlayer` / `ITrackSearcher` / `ISpotifyController` / `IResearchSource` / `IClock`
...
- `AIRadio.Infrastructure`: `SystemClock`（実 `IClock`）
...
- `dotnet build` 成功
- `dotnet test` 全グリーン
- **製品コードの外部 NuGet 依存ゼロ**（`Core` / `Infrastructure` は標準ライブラリのみ。`App` は Avalonia のみ。テストは xUnit）
