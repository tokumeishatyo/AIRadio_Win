...
- `WebApiSpotifyController : ISpotifyController`（Infrastructure）
  ...
  - デバイス選択: `preferredDeviceName` 指定時は名前一致のみ（なければ NoDevice）。未指定時は `type=Computer`（アクティブ優先、なければ最初の Computer）。**`devices.first` への安易なフォールバックはしない**（スマホ等へ転送して別の場所で鳴らす事故防止）。
...
- テスト: `WebApiSpotifyControllerTests`（device 解決・Computer 優先・preferredDeviceName・NoDevice・volume/seek/pause・pause 例外握り潰し・playerState パース・204=Stopped・404→transfer 再試行・404 継続失敗）／ `WaitForTrackToFinishTests`（自然終端・stale 混入で切らない・確定 trackChanged・stale 曲長で早切りしない・切替直後 stale スキップ・スナップショット曲長なし時の fallback）。
