...
| `E-TTS-AQT-LOAD-FAILED-001` | TTS | AquesTalk / AqKanji2Koe の DLL ロード失敗（不在・x86/x64 不一致）→ 当該セグメントスキップ（fail-tolerant）| W-AQT |
| `E-TTS-AQT-INIT-FAILED-001` | TTS | AqKanji2Koe_Create 失敗（辞書 path 不正・破損）→ composition で握り潰し当該 DJ 無音（fail-tolerant）| W-AQT |
| `E-TTS-AQT-KANA-FAILED-001` | TTS | AqKanji2Koe_Convert 失敗（漢字→音声記号列）→ 当該セグメントスキップ（fail-tolerant）| W-AQT |
| `E-TTS-AQT-SYNTHESIS-FAILED-001` | TTS | AquesTalk_Synthe が NULL（記号列不正等）→ 当該セグメントスキップ（fail-tolerant）| W-AQT |

> AquesTalk1（W-AQT）の TTS エラーは**実行時 fail-tolerant**（throw → 非 critical セグメント握り潰しで放送継続）。設定欠落（`voices_dir`/`dict_dir`/`aquestalk_voice`・speaker_id 一意性違反）は `E-CFG-MISSING-FIELD-001` で fail-fast。dev key 未設定は throw せず WARN ログのみ（評価版動作＝ナ/マ行が「ヌ」化）。
