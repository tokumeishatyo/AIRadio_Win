...
| 単一インスタンス | 名前付き `Mutex`（`SingleInstance`・存在検出）| W-Win 実装済（2 トレイの競合防止。トレイのみ・CLI デモ対象外）|
| VOICEVOX 自動起動 | `VoicevoxLauncher`（probe `GET /version` → 未応答なら起動。tts.yaml `voicevox.exe_path` 設定優先・未設定は標準インストール先探索）| W-Win 実装済。**アプリ自身のログイン時自動起動はユーザー判断で不実装**（旧 §2 案の `HKCU\Run` 自動起動から方針変更）。完全 fail-tolerant |
| エラー通知（`NSAlert`） | Avalonia メッセージボックス / トレイバルーン | |
| **AquesTalk（Windows 専用追加）** | **AquesTalk1（初代）**。`ITTSBackend` 実装としてスロットイン。**最小・非破壊の Core 追加**（DjProfile に任意2フィールド `tts_backend`/`aquestalk_voice`。W-AQT spec §3-4） | **【最後に実装】** §9 参照（旧版の AquesTalk10 P/Invoke は流用不可）|
...
│   └── (後で) Tts/RoutingTTS + AquesTalk1Synthesizer + AquesTalk1Native (P/Invoke)
...
| | W11 | ニュースの LLM 会話化（news seam はデリゲート維持・`IAnnouncementProvider` を作らない＝2 実装目でも単一消費者のため §3-5, 2026-06-16 確定）|
| 番組構造 | W12 | 季節コンテキスト + テーマプール + お便りコーナー（乱数 seam=`Func<int,int>` クロージャ・新 `IRandom` を作らない＝§3-5、format 分岐は `CornerEngine` 内・`LetterCornerEngine` を作らない、dateContext=`SeasonPhrases.DateContext`〔記念日版 DailyCalendar は W17〕, 2026-06-16 確定）|
...
| | W19b | アーティスト読み自動登録 |
| Windows 固有 | **W-Win** | 単一インスタンス（Mutex）、VOICEVOX 自動起動（probe→起動）、graceful shutdown（明示終了＝W9 ＋ セッション終了 `ShutdownRequested` ベストエフォート）。docs/specs/w-win-lifecycle.md |
| **最後** | **W-AQT** | **AquesTalk1 TTS バックエンド**（`ITTSBackend` にスロットイン、**最小・非破壊の Core 追加**＝DjProfile に任意2フィールド）|
...
- **配布形態と autostart**: zip / インストーラ、起動方式 → Windows 固有スライスで決定。**アプリ自身のログイン時自動起動はユーザー判断で不実装（W-Win）**。
- **VOICEVOX 自動起動**: Mac は BYO 手動。Windows 差別化機能として **W-Win で実装済**（`VoicevoxLauncher`＝probe `GET /version` → 未応答なら起動。tts.yaml `voicevox.exe_path` 設定優先・未設定は標準インストール先探索・完全 fail-tolerant）。
