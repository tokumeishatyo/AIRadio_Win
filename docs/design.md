...
| | W2 | Spotify Web API 検索 + `IsPlayable` プレフライト |
| | W3 | 統一 ThemeSequencer（OP/news/ED の BGM ダッキング演出）|
| | W4 | Spotify Web API 再生 + OAuth PKCE + LoopbackServer(HttpListener) + DPAPI トークン保管 |
...
| | W7 | BroadcastEngine（番組進行、`program.yaml`、fail-tolerant/critical）|
| | W8 | 時刻・日付アナウンス + 音量/話速 |
| シームレス | W10 | 切れ目ない放送（冒頭曲セグメント + ローリング先読み準備）|
| | W11 | ニュースの LLM 会話化 |
...
- 技術: .NET 10 + Avalonia 12 / 音声=NAudio / YAML=YamlDotNet（汎用 Loader 1 個）/ トークン=DPAPI / 単一インスタンス=名前付き Mutex。
