...
/// W4 <see cref="ThemeSequencer"/> がローダを消費者（= W7 <see cref="BroadcastEngine"/>）へ委譲したもの。W7 は anchor が
/// 読む単一・フラット形（曜日替わり DJ の <c>by_dj</c> は W13.5）。news の announcement は実行時のニュース原稿で
/// 差し替えるため読まない。<c>track_uri</c> 欠落は fail-fast（<c>E-CFG-MISSING-FIELD-001</c>）。
/// </summary>
